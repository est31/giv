...
- show tags/branches
- show dirty state as separate 'commit' (diff worktree to index)
  - done via `Repository::status`
- show index state as separate 'commit' (diff index to commit)
  - `git_diff_tree_to_index` function in libgit2, [not available in gitoxide](https://github.com/GitoxideLabs/gitoxide/issues/301)
  - can hack this via `git write-tree`
